...
export default function Diagnostico() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const rotinas =
        (await supabase.from("rotinas").select("id, nome")).data ?? [];
        ...
        sessoes.length === 0
          ...
          : (((
              await supabase
                .from("execucoes")
                .select(
                  "id, sessao_id, exercicio_id, concluido, carga, reps_feitas",
                )
                .in(
                  "sessao_id",
                  sessoes.map((x) => x.id),
      ...
      const dias: DiaNuvem[] = sessoes.map((x) => {
        ...
        const comCarga = lista.filter(
          (e) => e.carga != null || e.reps_feitas != null,
        ).length;
        return {
          id: x.id,
          data: x.data,
      ...
      const idsParaConsertar = dias
        ...
        .flatMap((d) =>
          (porSessao.get(d.id) ?? [])
            .filter(
              (e) => !e.concluido && (e.carga != null || e.reps_feitas != null),
            )
            .map((e) => e.id),
        );
  ...
  async function consertar() {
    ...
    try {
      ...
      setAviso(
        `Pronto: ${mexidos} exercício(s) voltaram a contar como feitos.`,
      );
      await carregar();
    } catch (e) {
    ...
    <div className={s.page}>
      ...
      {info.recuperaveis > 0 && (
        <div className={s.alerta}>
          <b>
            {info.recuperaveis} dia(s) com carga registrada e nenhum exercício
            marcado como feito.
          </b>{" "}
          É o efeito do bug antigo em que anotar a carga desmarcava o
          "concluído" — o treino está salvo, mas o calendário não pinta o dia
          porque ele conta exercícios concluídos.
          <div className={s.acoes}>
            ...
            >
              {consertando
                ? "Consertando..."
                : "Marcar como feitos os que têm carga"}
            </button>
          </div>
        </div>
      )}
      ...
      <p className={s.fraco}>
        {info.nuvem.rotinas} rotinas · {info.nuvem.sessoes} dias ·{" "}
        {info.nuvem.execucoes} registros
      </p>
      ...
      <p className={s.fraco}>
        {info.local.rotinas} rotinas · {info.local.sessoes} dias ·{" "}
        {info.local.execucoes} registros
      </p>
